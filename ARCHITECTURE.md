...
# ADR-019 — Деньги и НЕИЗМЕНЯЕМЫЕ versioned campaign terms (правка по ревью 2026-09-22)
Денежный контракт: суммы `Numeric(14,2)`, CPM-ставки `Numeric(10,4)`, проценты `Numeric(5,2)`; только `Decimal` (float запрещён audit-правилом); rounding ROUND_HALF_EVEN до сотых только на границах записи; валюта ISO-4217 на кампанию; **gross / creator fee / net хранятся раздельно** (creator fee ≠ brand fee). Изменчивые правила платформы — отдельная НЕИЗМЕНЯЕМЫЕ таблица `campaign_terms_versions` (только INSERT, `content_hash`, uq (campaign_id, version), подтверждение пользователем); `reward_campaigns` держит лишь указатель `current_terms_version_id`; каждый `reward_submissions` фиксирует `terms_version_id`, действовавший на момент сабмишена. Дефолты термов — из KNOWLEDGE §7 (10% CPM / 30 мин / 7 дней / 3 дня), но подтверждаются при импорте. Ни одно из этих чисел не хардкодится в логике.
