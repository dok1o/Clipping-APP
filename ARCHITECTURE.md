...
- Stage 8 / YouTube (реализовано): `services/publish/youtube.py` — официальный resumable-протокол поверх raw httpx. ADR-017: google-api-python-client НЕ добавлен (тяжёлая зависимость ради двух HTTP-вызовов); single-shot PUT на resumable-сессии протокольно корректен для размеров клипов, чанковая дозагрузка — опциональное расширение. Общий `resolve_access_token` (Google OAuth2 refresh) переиспользуется metrics-адаптером. Реестр publisher'ов в base.py — добавление платформы не трогает ядро.
...
# ADR-019 — Деньги и НЕИЗМЕНЯЕМЫЕ versioned campaign terms (правка по ревью 2026-09-22; уточнение v2.1 2026-09-23; реализация CR-1 2026-09-23)
Денежный контракт: ВСЕ суммы `Numeric(14,2)` (v2.1: унифицировано, включая expected fee), CPM-ставки `Numeric(10,4)`, проценты `Numeric(5,2)`; только `Decimal` (float запрещён audit-правилом); rounding ROUND_HALF_EVEN до сотых только на границах записи; валюта ISO-4217 на кампанию; **gross / creator fee / net хранятся раздельно** (creator fee ≠ brand fee), expected и actual — по три поля каждой компоненты (`payout_gross`/`fee_amount`/`payout_net` × `expected`/`actual`). Изменчивые правила платформы — отдельная НЕИЗМЕНЯЕМАЯ таблица `campaign_terms_versions` (только INSERT, `content_hash`, uq (campaign_id, version), подтверждение пользователем) — ЕДИНСТВЕННОЕ хранилище экономических термов: `payout_model`, `cpm_rate`, `per_post_amount`, `retainer_amount`/`retainer_cycle_days`, `min_payout`/`max_payout_per_clip`, fee, окна (v2.1: дублирующие поля из `reward_campaigns` убраны — в кампании остаются только идентичность/статус/снимки бюджета и указатель `current_terms_version_id`); каждый `reward_submissions` фиксирует `terms_version_id`, действовавший на момент сабмишена. Дефолты термов — из KNOWLEDGE §7 (10% CPM / 30 мин / 7 дней / 3 дня), но подтверждаются при импорте. Ни одно из этих чисел не хардкодится в логике. Реализация CR-1: (а) в JSON API денежные поля сериализуются ТОЧНЫМИ строками decimal (`"10.0000"`), не float — точность Numeric не теряется на границе HTTP; (б) импорт кампании детерминирован по (provider, external_campaign_id): повторный импорт обновляет операционный снимок и НЕ создаёт дублей кампании/версий термов/брифа (идентичный контент — тот же content_hash → reuse); (в) циклические указатели `current_terms_version_id`/`active_brief_version_id` — FK SET NULL, создаются ALTER-ом в конце миграции 0007 (batch для SQLite); (г) provider str32 (правка контракта: str16 не вмещал каноническое значение `whop_content_rewards`).
