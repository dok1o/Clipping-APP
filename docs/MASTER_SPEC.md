...
- Первая платформа — **TikTok** (выбор пользователя 2026-09-20, до старта T1.4): реализуется через официальный Content Posting API. YouTube — вторая платформа, подключается позже; архитектура `services/publish/` (интерфейс Publisher) обязана допускать это без рефакторинга ядра.
