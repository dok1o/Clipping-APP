...
# ЧАСТЬ CR — Content Rewards (creator monetization), Stage CR-0..CR-9 — v2.1 (правки по ревью 2026-09-22 и 2026-09-23)

> Зафиксировано 2026-09-22 по директиве пользователя; **v2 — после репозиторной ревь**ю: versioned terms вынесены в неизменяемую таблицу, добавлен audit-log переходов, закреплён денежный контракт, уточнены факты fee/scopes. **v2.1 (2026-09-23) — три дефекта ревью v2 исправлены**: (1) экономические термы убраны из `reward_campaigns` — `payout_model`/ставки/суммы/min-max живут только в `campaign_terms_versions`; (2) все денежные суммы — `NUMERIC(14,2)`, добавлен `fee_amount_expected`; (3) `expired` — только из `submission_required`, не из `posted`. **Код CR-1+ не начинается до согласования этой версии контракта** (гейт CR-0). Продукт: creator-only, Whop-first, provider-neutral, USD-first (ISO currency ready). Главные KPI: фактическая чистая подтверждённая выплата; ожидаемая чистая выплата за клип; выплата за час активной работы; approval rate; доля отправленных вовремя ссылок; доля клипов без нарушений brief. Просмотры — промежуточный сигнал.
...
**`reward_campaigns`**: `id` PK; `provider` str32 (`whop_content_rewards`; правка CR-1 2026-09-23: было str16 — блокирующее противоречие, каноническое значение из этого же контракта не помещалось в 16 символов; выровнено с `reward_submissions.provider` str32); `external_campaign_id` str128; **uq (provider, external_campaign_id)**; `name` str255; `brand_name` str255 null; `source_url` str1024 (обязателен при импорте); `status` str16 (`draft|active|paused|closed|unknown`); `platforms` json ⊆ `tiktok|youtube|instagram|x`; `currency` char3 ISO-4217 default `USD`; `budget_total`/`budget_spent` NUMERIC(14,2) null (операционный снимок бюджета, не realtime и НЕ термы); `deadline_at` timestamptz null (календарь кампании, не термы); `current_terms_version_id` FK→campaign_terms_versions null (ставится при подтверждении термов пользователем); `active_brief_version_id` FK→campaign_brief_versions null (только действием пользователя); `imported_payload` json; timestamps. **Экономических термов полями кампании НЕТ (v2.1 по ревью)**: `payout_model`, `cpm_rate`, `per_post_amount`, `retainer_amount`, `retainer_cycle_days`, `min_payout`, `max_payout_per_clip` — ТОЛЬКО в `campaign_terms_versions` (единственный источник термов = версия); кампания хранит идентичность/статус/платформы/валюту/снимки бюджета/указатели версий. FK `current_terms_version_id` создаётся ALTER-ом в конце 0007 (циклическая ссылка campaign ↔ terms внутри одной миграции).

**`campaign_terms_versions`** — НЕИЗМЕНЯЕМЫЕ versioned campaign terms, ЕДИНСТВЕННОЕ хранилище экономических условий (v2.1: сюда перенесены `payout_model` и все ставки/суммы; ревью: «нельзя просто изменять в reward_campaigns»): `id` PK; `campaign_id` FK CASCADE; `version` int ≥1 **uq (campaign_id, version)**; `content_hash` char64 (sha256 канонического JSON термов — контроль неизменности); `payout_model` str16 (`cpm|per_post|retainer`); `cpm_rate` NUMERIC(10,4) null (для cpm); `per_post_amount` NUMERIC(14,2) null (для per_post); `retainer_amount` NUMERIC(14,2) null + `retainer_cycle_days` int null (для retainer); `min_payout`/`max_payout_per_clip` NUMERIC(14,2) null; `creator_fee_percent` NUMERIC(5,2); `fee_free_budget_threshold` NUMERIC(14,2) (per-post/retainer с бюджетом ≥ порога → 0%; дефолт 5000 по KNOWLEDGE §7, но подтверждается при импорте); `earnings_window_days` int; `payout_hold_days` int; `submission_deadline_minutes` int; `terms_source_url` str1024; `terms_checked_at` timestamptz; `confirmed_at` timestamptz null (подтверждение пользователем); `created_at`. Заполнены только поля, релевантные `payout_model` (валидируется при INSERT; нерелевантные — null). Строки только INSERT; изменение термов = новая версия; снапшоты/сабмишены/forecast ссылаются на конкретную версию; `reward_campaigns.current_terms_version_id` — лишь указатель на подтверждённую.

**`campaign_brief_versions`**: `id` PK; `campaign_id` FK; `version` int **uq (campaign_id, version)**; `status` (`pending_approval|approved|rejected|superseded`) — автоактивации нет; источник: `raw_text` text null | `raw_file_key` str512 null | `structured_json`; `source_url` str1024; `checklist` json (CR-2: `{requirement_id, category, rule, expected, severity_hint, confidence, source_span, extracted_value}`); `parser_engine` str32; `parser_confidence` NUMERIC(4,3); `supersedes_id` FK self null; `approved_at` null; `created_at`. Новая approved версия → ранее проверенные клипы stale (re-run compliance).
...
**`reward_submissions`**: `id` PK; `campaign_clip_id` FK; `campaign_id` FK; `terms_version_id` FK **NOT NULL** (термы, действовавшие на момент сабмишена); `provider` str32; `status` (CR-0.3); `idempotency_key` str255 **uq**; `post_url` str1024; `posted_at` timestamptz; `posted_render_sha256` char64; `submission_deadline_at` = posted_at + terms.submission_deadline_minutes; `submitted_at` null; `submitted_via` (`manual|api`); `external_submission_id` str255 null; `rejection_reason` text null; деньги (CR-0.4): `payout_gross_expected`/`fee_amount_expected`/`payout_net_expected` NUMERIC(14,2) (forecast на решение «постить»; по terms_version, net = gross − fee), `payout_gross_actual`/`fee_amount_actual`/`payout_net_actual` NUMERIC(14,2) null (факт, вводится пользователем/провайдером); `currency` char3; `paid_at` null; `last_error` text null; timestamps. Индексы: (campaign_id, status), (status, submission_deadline_at).
...
**`reward_snapshots`**: `id` PK; `submission_id` FK CASCADE; `captured_at` timestamptz; `views` bigint null; `views_kind` (`platform_official|provider_reported|user_reported`); `payout_gross_cumulative`/`payout_net_cumulative` NUMERIC(14,2) null; `raw` json. Индекс (submission_id, captured_at).
...
- **Gross / creator fee / net** (раздельно, по ревью): `payout_gross` — начислено до комиссии; `fee_amount` — creator fee по terms_version (для Content Rewards: CPM всегда 10%; per-post/retainer с бюджетом ≥ threshold → 0%; creator fee ≠ brand/platform fee — не смешивать); `payout_net = payout_gross − fee_amount` — то, что попадает в wallet. Ожидание (expected) и факт (actual) — раздельные поля для КАЖДОЙ компоненты gross/fee/net (`payout_gross_expected`/`fee_amount_expected`/`payout_net_expected` и `payout_gross_actual`/`fee_amount_actual`/`payout_net_actual`; v2.1 — добавлен expected fee); снапшоты хранят кумулятив gross и net.
...
| submission_required | expired | system (submission_deadline_at прошёл без submitted) | submission_deadline_at |
...
Пояснение (v2.1): `posted` — кратковременное техническое состояние (система сразу переводит его в `submission_required`, стартуя countdown), поэтому `expired` достижим ТОЛЬКО из `submission_required` — по наступлению `submission_deadline_at` без `submitted`. После `submitted` сабмишен не «истекает». `expired` — терминальное состояние; новая попытка по тому же клипу = новый `reward_submissions` (и только если кампания ещё принимает).
...
Источник параметров (v2.1): `cpm_rate`/`per_post_amount`/`retainer_amount`/`min_payout`/`max_payout_per_clip`/fee% — из `campaign_terms_versions` (версия термов сабмишена либо последняя подтверждённая на момент решения «постить»); `budget_total`/`budget_spent` — из `reward_campaigns` (операционный снимок).
