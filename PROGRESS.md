...
3. **Денежный контракт** (`backend/app/services/rewards/money.py`): `to_decimal` (float отклоняется — MoneyError), `quantize_money` (ROUND_HALF_EVEN до сотых, только на границах записи), `compute_fee`/`compute_net` (полная точность), `terms_content_hash` (sha256 канонического JSON: sorted keys, compact separators, Decimal без экспоненты).
4. **Сервис** (`backend/app/services/rewards/campaign_service.py`): детерминированный импорт по (provider, external_campaign_id) — повторный импорт обновляет операционный снимок (name/status/platforms/budget/deadline/imported_payload/source_url), НЕ создаёт дублей; идентичные термы (тот же content_hash) → reuse версии, изменённые → version N+1; подтверждение термов — отдельное явное действие (`confirmed_at` + указатель, идемпотентно); брифы создаются ТОЛЬКО как pending_approval, approve/reject — отдельные явные PATCH-действия (approve: прежняя approved → superseded, `active_brief_version_id` = версия); ассеты authorized=False по умолчанию; никакой автоактивации, никаких сетевых вызовов, никакого scraping.
5. **API** (`backend/app/api/v1/rewards.py`, тонкий слой): POST `/reward-campaigns/import` (JSON), POST `/reward-campaigns/import-text` (text), POST `/reward-campaigns/import-file` (multipart: .json = полный import-запрос; .txt/.md = текст брифа + form-поля кампании; файл сохраняется в S3 `briefs/{campaign_id}/{sha256[:16]}-{name}` → `raw_file_key`); GET list/detail; GET/POST/PATCH `/{id}/terms` (история/новая версия/подтверждение); GET/POST/PATCH `/{id}/brief` (история/новая pending_approval/approve-reject). Ошибки — единый формат §1 (422 `invalid_campaign_terms` с fields, 422 `invalid_import_file`, 415 `unsupported_media`, 404 `campaign_not_found`/`terms_version_not_found`/`brief_version_not_found`, 409 `invalid_brief_state`).
6. **Контрактная правка (единственная, блокирующее противоречие)**: `reward_campaigns.provider` str16 → **str32** — каноническое значение `whop_content_rewards` (21 символ) из того же CONTRACTS v2.1 не помещалось в 16 символов; выровнено с `reward_submissions.provider` str32 (правка внесена в CONTRACTS с фиксацией причины, ADR-019 п. г).

Структура: models/rewards.py; schemas/rewards.py (Decimal-поля с max_digits/decimal_places; в ответах деньги — точные decimal-строки, не float); services/rewards/{__init__,money,campaign_service}.py; api/v1/rewards.py; миграция 0007. Экономических терм-полей в reward_campaigns НЕТ (тест инспектирует колонки миграции и модели).
API/schema changes: +11 эндпоинтов выше; миграция 0007 (head); CONTRACTS — только provider str32 (блокирующее противоречие).
Dependencies: нет новых (httpx/starlette уже в проекте; S3 — существующий moto в тестах).
Tests и фактический вывод (песочница, окружение .venv из bf5ffeb-состояния): baseline на 9680966 = `pytest -m "not real_services"` → **207 passed, 3 deselected**; после CR-1 → **309 passed, 3 deselected** (+102: 12 money, 30 моделей, 29 сервис, 30 API, 1 миграции-колонки). Миграции: `alembic upgrade head` → 0006→0007; `alembic downgrade -1` → 0007→0006; `alembic upgrade head` → снова 0007; `alembic current` → **0007 (head)**; parity «миграции ↔ Base.metadata» зелёный (включая кортежи колонок/uq/idx/fk/cks). `git diff --check` — чисто. Frontend не затронут (npm-проверки не требуются по условию «если затронут»).
Git status --short после коммита: чисто (единственный атомарный коммит `cr1: add campaign foundation and brief import`).
Diff summary: +9 новых файлов backend (модель/схемы/сервис×3/роутер/миграция/4 тест-файла), правки router.py, models/__init__.py, test_migrations.py, CONTRACTS.md (provider str32), ARCHITECTURE.md (ADR-019 п. а–г), PROGRESS.md.
Known limitations: S3-сохранение файла брифа тестируется на moto (реальный MinIO — на машине пользователя); import-file для .json не сохраняет сам файл (структура уже в imported_payload — зафиксировано в коде); PATCH /terms с confirm=false → 422 (единственный валидный путь — подтвердить).
